...
def main():
    ...
    for i,r in enumerate(at):
        out.append('| %d | %s | ¥%d | %d | %+.0f%% | %s | ¥%d | ¥%d | ¥%d | ¥%d | %.1f:1 |'%(
            i+1,str(r[1]),int(r[3]),int(r[4]),float(r[5]),str(r[6]),
            int(r[9]),int(r[10]),int(r[11]),int(r[12]),float(r[13])))
    
    out.append('\n## 🇭🇰 港股\n### 🟢 买入\n| 标的 | 价格 | 评分 | 年涨 | 信号 | 中枢 |\n|------|------|------|------|------|------|')
    for r in hb[:5]:
        out.append('| %s | HK$%d | %d | %+.0f%% | %s | %s |'%(str(r[1]),int(r[3]),int(r[4]),float(r[5]),str(r[2]),str(r[6])))
    out.append('\n### 🔴 规避\n| 标的 | 价格 | 信号 |\n|------|------|------|')
    for r in hs[:5]:out.append('| %s | HK$%d | %s |'%(str(r[1]),int(r[3]),str(r[2])))
