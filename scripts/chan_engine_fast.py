#!/usr/bin/env python3
"""chan_engine_fast.py — chan.py batch mode wrapper (faster than trigger_step)"""
import re,os,sys
...
# Non-trigger_step config — batch mode is much faster
CONFIG=CChanConfig({
    'trigger_step': False,
    'divergence_rate': 0.7,
    'min_zs_cnt': 0,
...
def analyze(dates, opens, closes, highs, lows, code='unknown'):
    """Run chan.py analysis in batch mode (faster)."""
    n=len(dates)
    ...
    for i in range(n):
        ...
        klines.append(CKLine_Unit({
            DATA_FIELD.FIELD_TIME: t,
            DATA_FIELD.FIELD_OPEN: opens[i],
            DATA_FIELD.FIELD_HIGH: highs[i],
            DATA_FIELD.FIELD_LOW: lows[i],
            DATA_FIELD.FIELD_CLOSE: closes[i],
        }, autofix=True))
    
    # Batch mode: feed all K-lines at once
    chan=CChan(code=code, begin_time=None, end_time=None,
               data_src=1, lv_list=[KL_TYPE.K_DAY], config=CONFIG)
    chan.trigger_load({KL_TYPE.K_DAY: klines})
    cur=chan[0]
